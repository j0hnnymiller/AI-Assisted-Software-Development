...
The single pipeline entry point is `.github/prompts/merge-marp-decks.prompt.md`, a
...
1. **First Section Exception**: The first section (idx=0) MUST NOT receive injected slides (module list, section header, agenda). These slides are only added for idx > 0.
2. **Slide Notes**: ALL slides (injected and content) MUST have speaker notes explaining their source/purpose:
   - Injected slides: Notes explaining they were auto-generated and their purpose
   - Content slides: Notes showing source file path
...
- Verify first section has NO injected slides in the output PPTX
- Verify ALL slides (including injected) have speaker notes in the output PPTX
...
4. [Merge phase — injected slides](#4-merge-phase--injected-slides)
   ...
   - 4.2 [Section header slide](#42-section-header-slide)
   - 4.3 [Section agenda slide](#43-section-agenda-slide)
...
- A `slides:` list may be empty or absent; the section still produces a module list slide,
  a section header slide, and an empty section grouping in the PPTX.
...
| H1 headings        | No `# H1` headings in the body; H1s are reserved for section header slides generated by the pipeline                                                                                                                 |
...
During the merge the agent inserts three auto-generated slides at the start of each
...
### 4.2 Section header slide

**One section header slide is inserted after the module list slide for every section.**

Its purpose is to announce the section with a visually prominent title.
...
where `Section Name` is the exact `name` value from the manifest.

---

### 4.3 Section agenda slide

**One section agenda slide is inserted after the section header slide for every section
...
After the three injected slides, the source files for the section are merged in order.
...
│      (# Section Name, lead class)                    │
├──────────────────────────────────────────────────────┤
│  3.  Section agenda slide          ← omitted when    │
│      (## Section Name + bullet     ← section has     │
│       list of slide titles)        ← no source files │
├──────────────────────────────────────────────────────┤
│  4.  Content slides from source file 1               │
│  5.  Content slides from source file 2               │
│  …                                                   │
└──────────────────────────────────────────────────────┘
```
...
- For each section in the manifest: a named `<p14:section>` XML group in the PPTX
  covering all slides belonging to that section (module list + header + agenda + content).
- **Empty sections** (no source files) receive a section group containing only their
  two injected slides (module list + section header).
- **🔒 CRITICAL**: Speaker notes on EVERY slide:
  - **Injected slides**: Notes explaining auto-generation and purpose
    ...
    - Section header: "Auto-generated section header slide to announce the section"
...
**REQUIRED BEHAVIOR**: The first section in the YAML manifest (index 0) MUST NOT receive the three injected slides. This is implemented in `generate_pptx.py` with:
...
2. Subsequent sections DO have the three injected slides
...
```
1. Open .github/prompts/merge-marp-decks.prompt.md in VS Code
...
│      b. Section header slide  (# Section Name)                 │
...
| Empty sections        | Sections with no source files produce a module list slide, a section header slide, and an empty PPTX section group — no agenda slide.                                                                                              |
...
- [ ] **First Section Test**: Generate PPTX from a manifest where the first section has at least 2 content slides
  ...
  - [ ] Verify NO section header slide appears before the first content slide
...
- [ ] **Subsequent Sections Test**: Verify sections after the first
  ...
  - [ ] Section header slide appears after "Course Modules"
  - [ ] Section agenda slide appears after section header (if section has content slides)

- [ ] **Speaker Notes Test**: Open PPTX and verify EVERY slide has

speaker notes

- [ ] Module list slides: Note says "Auto-generated course navigation slide..."
- [ ] Section header slides: Note says "Auto-generated section header slide..."
...
4. Module 1 - Test (section header, has note "Auto-generated section...")
5. Module 1 - Test (section agenda, has note "Auto-generated section...")
6. What's the Big Deal (content slide, has note "Source: ...")
