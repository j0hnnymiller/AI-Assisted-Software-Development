...
[2025-10-15T14:32:00Z] anthropic/claude-3.5-sonnet@2024-10-22
...
**AUTHORITATIVE SOURCE**: Complete metadata requirements are defined in [`.github/instructions/ai-assisted-output.instructions.md`](ai-assisted-output.instructions.md).

**Summary**: Every AI-generated artifact MUST include all 11 required fields:

1. `ai_generated` (boolean)
2. `model` (provider/model@version format)
3. `operator` (GitHub username)
4. `chat_id` (chat identifier)
5. `prompt` (exact prompt text)
6. `started` (ISO8601 timestamp)
7. `ended` (ISO8601 timestamp)
8. `task_durations` (task breakdown array)
9. `total_duration` (total time)
10. `ai_log` (path to conversation log)
11. `source` (creation identifier)

**Template and Examples**: See [Standard Metadata Front Matter](ai-assisted-output.instructions.md#standard-metadata-front-matter) section.
